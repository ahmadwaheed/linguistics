...
go(CurrentState, InString, OutString) :-           % Getting standard In and Out puts
    transition(CurrentState, eps, NextState, eps), % From Curr to Next State
    go(NextState, InString, OutString).            % Goes to next state
